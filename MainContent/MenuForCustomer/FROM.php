...
$query_num_get_post = "SELECT COUNT(W.`ID-post`) FROM `Waybill` W, `Statuspost` S
                        WHERE W.`ID-post` = S.`ID-post` AND W.`ID-addressee` = '".$_COOKIE['Sender-ID']."' AND S.`StatusOfPost` = 'Доставлено'";


$a = mysqli_query($connectMySQL, $query_num_get_post);
